...
FileRateLimiter::FileRateLimiter(const std::filesystem::path& state_file, double rate,
                                 size_t burst_size, std::chrono::milliseconds timeout_ms)
    : state_file_(state_file), rate_(rate), burst_size_(burst_size), timeout_(timeout_ms)
{
...
void FileRateLimiter::reset()
{
    ...
    TokenBucketState state{0.0, // Empty bucket
                           std::chrono::duration_cast<std::chrono::milliseconds>(
                               std::chrono::system_clock::now().time_since_epoch())
                               .count(),
                           rate_, burst_size_, 1};
...
void FileRateLimiter::save_state(const TokenBucketState& state)
{
    ...
    writer.write(std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(&state),
